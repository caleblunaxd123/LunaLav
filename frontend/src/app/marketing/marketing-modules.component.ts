...
export class MarketingModulesComponent implements OnInit, OnDestroy {
  ...
  onImagen(e:any){const f=e.target.files?.[0];if(!f)return;if(!f.type.startsWith('image/')){this.iaMsg.set('El archivo debe ser una imagen.');return;}if(f.size>10*1024*1024){this.iaMsg.set('La imagen supera 10 MB.');return;}const r=new FileReader();r.onload=()=>this.imagenUrl.set(String(r.result||''));r.readAsDataURL(f);e.target.value='';}
  quitarImagen(){this.imagenUrl.set('');}
  descargarImagen(){const u=this.imagenUrl();if(!u)return;const a=document.createElement('a');a.href=u;a.download='publicacion-lunalav.png';document.body.appendChild(a);a.click();a.remove();}
