...
export class MarketingModulesComponent implements OnInit, OnDestroy {
  ...
  pub:any={tipo:'promo',temporada:'ninguna',negocio:'LunaLav',oferta:'',zona:'',contacto:'',emojis:true};postText='';copiado=signal(false);historial=signal<any[]>([]);private histKey='lunalav.pub.historial';iaLoading=signal(false);iaMsg=signal('');
  ...
  private readonly tempData:Record<string,{emoji:string,frase:string,tags:string[]}>={ninguna:{emoji:'',frase:'',tags:[]},navidad:{emoji:'🎄',frase:'Esta Navidad luce impecable.',tags:['#Navidad','#FelizNavidad']},anionuevo:{emoji:'🎆',frase:'Empieza el año con ropa como nueva.',tags:['#AñoNuevo']},fiestaspatrias:{emoji:'🇵🇪',frase:'¡Celebra el Perú con tu mejor look!',tags:['#FiestasPatrias','#Perú']},sanvalentin:{emoji:'❤️',frase:'Para esa cita especial, ropa impecable.',tags:['#SanValentín']},diamadre:{emoji:'💐',frase:'Regálale a mamá su tiempo: nosotros lavamos.',tags:['#DíaDeLaMadre']},blackfriday:{emoji:'🖤',frase:'¡Ofertas de Black Friday en lavado!',tags:['#BlackFriday','#Ofertas']}};
  ...
  private hashtags(){const t=['#Lavanderia','#RopaLimpia',`#${(this.pub.negocio||'LunaLav').replace(/[^\p{L}\p{N}]/gu,'')||'LunaLav'}`];const z=(this.pub.zona||'').replace(/[^\p{L}\p{N}]/gu,'');if(z)t.push('#'+z);if(this.pub.tipo==='promo')t.push('#Promo','#Descuento');if(this.pub.tipo==='consejo')t.push('#Tips');t.push(...(this.tempData[this.pub.temporada]?.tags||[]));return [...new Set(t)].join(' ');}
  ...
  generarIA(){if(this.iaLoading())return;this.iaLoading.set(true);this.iaMsg.set('Generando con IA local (puede tardar unos segundos)…');this.svc.generarPublicacionIA({tipo:this.pub.tipo,temporada:this.pub.temporada,negocio:this.pub.negocio,oferta:this.pub.oferta,zona:this.pub.zona,contacto:this.pub.contacto,emojis:this.pub.emojis}).subscribe({next:r=>{if(r?.texto)this.postText=r.texto;this.iaLoading.set(false);this.iaMsg.set('✔ Texto generado con IA. Puedes editarlo antes de publicar.');this.copiado.set(false);},error:e=>{this.iaLoading.set(false);this.iaMsg.set(e.error?.mensaje||'La IA local no está disponible ahora. Se mantiene el texto de plantilla.');}});}
