...
WHEN NOT MATCHED THEN INSERT(Address,Provider,Status,DisplayName,EncryptedCredentials,LastSyncAt) VALUES(N'contacto@lunalav.pe',N'GMAIL',N'CONNECTED',N'LunaLav Ventas',@cred,SYSUTCDATETIME());";
    ...
    private async Task SaveMessageAsync(string id,string thread,string direction,string? from,string? to,string subject,string? snippet,DateTime at,bool unread,CancellationToken ct){await using var c=db.Create();await c.OpenAsync(ct);await using var q=c.CreateCommand();q.CommandText=@"IF NOT EXISTS(SELECT 1 FROM communication.EmailMessage WHERE GmailMessageId=@id) BEGIN INSERT communication.EmailMessage(GmailMessageId,GmailThreadId,Direction,Sender,Recipient,Subject,Snippet,ReceivedAt,IsUnread) VALUES(@id,@t,@d,@f,@to,@s,@sn,@at,@u); MERGE communication.EmailThread AS x USING(SELECT @t Id) a ON x.GmailThreadId=a.Id WHEN MATCHED THEN UPDATE SET Subject=@s,LastSnippet=@sn,LastMessageAt=@at,IsUnread=@u WHEN NOT MATCHED THEN INSERT(GmailThreadId,ContactEmail,ContactName,Subject,LastSnippet,LastMessageAt,IsUnread) VALUES(@t,@f,@f,@s,@sn,@at,@u); END";q.AddParam("@id",id);q.AddParam("@t",thread);q.AddParam("@d",direction);q.AddParam("@f",from);q.AddParam("@to",to);q.AddParam("@s",subject);q.AddParam("@sn",snippet);q.AddParam("@at",at);q.AddParam("@u",unread);await q.ExecuteNonQueryAsync(ct);}
}
