...
public class MarketingProspectsController : ControllerBase
{
    ...
    [HttpGet("prospects")]
    public async Task<ActionResult<object>> Listar([FromQuery] string? q, [FromQuery] string? estado, [FromQuery] string? distrito, [FromQuery] int page=1, [FromQuery] int pageSize=25, CancellationToken ct=default)
    {
        ...
        cmd.CommandText=@";WITH P AS(SELECT p.*, ROW_NUMBER() OVER(ORDER BY p.Score DESC,p.FechaCreacion DESC) rn,COUNT(*) OVER() Total FROM marketing.Prospect p WHERE p.Activo=1 AND (@q IS NULL OR p.NombreComercial LIKE N'%'+@q+N'%' OR p.Telefono LIKE N'%'+@q+N'%' OR p.Distrito LIKE N'%'+@q+N'%') AND (@estado IS NULL OR p.Estado=@estado) AND (@distrito IS NULL OR p.Distrito=@distrito)) SELECT p.Id,p.NombreComercial,p.Distrito,p.Telefono,p.Whatsapp,p.Email,p.Estado,p.Prioridad,p.Score,p.Fuente,p.FechaUltimoContacto,(SELECT MIN(f.FechaProgramada) FROM marketing.FollowUp f WHERE f.ProspectId=p.Id AND f.Completado=0),p.Total FROM P p WHERE p.rn BETWEEN @from AND @to";
        cmd.AddParam("@q",string.IsNullOrWhiteSpace(q)?null:q.Trim());cmd.AddParam("@estado",string.IsNullOrWhiteSpace(estado)?null:estado.Trim().ToUpperInvariant());cmd.AddParam("@distrito",string.IsNullOrWhiteSpace(distrito)?null:distrito.Trim());cmd.AddParam("@from",(page-1)*pageSize+1);cmd.AddParam("@to",page*pageSize);
        var items=new List<MarketingProspectListItemDto>();var total=0;await using var rd=await cmd.ExecuteReaderAsync(ct);while(await rd.ReadAsync(ct)){total=rd.GetInt32(12);items.Add(new(rd.GetInt64(0),rd.GetString(1),rd.GetNullableString("Distrito"),rd.GetNullableString("Telefono"),rd.GetNullableString("Whatsapp"),rd.GetNullableString("Email"),rd.GetString(6),rd.GetString(7),rd.GetInt32(8),rd.GetString(9),rd.GetNullableDateTime("FechaUltimoContacto"),rd.GetNullableDateTime("Expr1"),[]));}
